...
describe('Question', function () {
  ...
  it('should return validation error when answer body is missing', function * () {
    const user = yield use('App/Model/User').find(1)
    const response = yield request(apiUrl)
    ...
    assert.equal(response.body.status, 400)
    assert.equal(response.body.message, 'Validation failed')
    assert.deepEqual(response.body.fields, [{field: 'body', message: 'Write your answer', validation: 'required'}])
  })

  it('should return 404 when unable to find question', function * () {
    const user = yield use('App/Model/User').find(1)
    const response = yield request(apiUrl)
      .post('questions/1/answers.json')
      .login(user)
      .send({body: 'I like the question'})
      .expect('Content-Type', /json/)
      .expect(404)
    assert.equal(response.body.status, 404)
    assert.equal(response.body.message, 'Cannot find question with given id')
  ...
  it('should return 200 when valid question id is passed', function * () {
    const user = yield use('App/Model/User').find(1)
    const question = yield use('App/Model/Question').create({title: 'Adonis 102', body: 'This is a question', user_id: 1})
    const response = yield request(apiUrl)
      .post(`questions/${question.id}/answers.json`)
      .login(user)
      .send({body: 'I like the question'})
      .expect('Content-Type', /json/)
      .expect(200)
    assert.equal(response.body.status, 200)
    assert.equal(response.body.message, 'Answer added successfully')
    assert.equal(response.body.data.question_id, question.id)
    assert.equal(response.body.data.body, 'I like the question')
  })

  it('should return 404 when unable to find question to which answers should be pulled', function * () {
    const response = yield request(apiUrl)
  ...
  it('should return 200 with zero answers when question id is valid but does not have answers', function * () {
    const question = yield use('App/Model/Question').create({title: 'Adonis 102', body: 'This is a question', user_id: 1})
    const response = yield request(apiUrl)
  ...
  it('should return all related answers to a given question', function * () {
    const question = yield use('App/Model/Question').create({title: 'Adonis 102', body: 'This is a question', user_id: 1})
    yield question.answers().createMany([{body: 'I like it'}, {body: 'You like it'}])
    const response = yield request(apiUrl)
      .get(`questions/${question.id}/answers.json`)
      .expect('Content-Type', /json/)
      .expect(200)
    assert.isArray(response.body.data)
    assert.equal(response.body.data[0].question_id, question.id)
    assert.equal(response.body.data[1].question_id, question.id)
